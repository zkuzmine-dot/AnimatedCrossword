...
interface CrosswordGridProps {
  ...
  status: {
    message: string;
    type: "success" | "error" | "warning" | "hidden";
  };
  onWordComplete: (word: string) => void;
  onWordIncomplete: (word: string) => void;
  onCheckAnswers: (correctCells: number, incorrectCells: number, emptyCells: number) => void;
  onResetPuzzle: () => void;
}
...
export default function CrosswordGrid({
  ...
  const [grid, setGrid] = useState<{[key: string]: string}>({}); // Stores user input
  const [cellStates, setCellStates] = useState<{[key: string]: 'correct' | 'incorrect' | ''}>({}); // Stores the state of each cell (correct, incorrect, or empty)
  ...
  useEffect(() => {
    // Check all words
    const checkWord = (startRow: number, startCol: number, direction: string, word: string) => {
      let isComplete = true;
      let row = startRow;
      let col = startCol;
  ...
  useEffect(() => {
    if (activeClue) {
      const input = document.querySelector(`input[data-row="${activeClue.row}"][data-col="${activeClue.col}"]`) as HTMLInputElement;
      if (input) {
        input.focus();
      }
    }
